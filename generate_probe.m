function probe = generate_probe(fs)
if nargin<1
    fs = 4000;
end
...
noise_2 = 2.0 * (heaviside(t-2) - heaviside(t-2.1)) .* rand(1, length(t));
noise_3 = 0.5 * (heaviside(t-3) - heaviside(t-3.3)) .* rand(1, length(t));
noise_4 = 0.5 * (heaviside(t-4) - heaviside(t-4.3)) .* rand(1, length(t));

harmochirp =       sin(2*pi*1*f1*(1+0.1*sin(2*t)) .* t) + ...
             0.3 * sin(2*pi*2*f1*(1+0.1*sin(2*t)) .* t) + ...
             0.1 * sin(2*pi*3*f1*(1+0.1*sin(2*t)) .* t);
chirp_1 = 1.0 * (heaviside(t-0.5) - heaviside(t-1.5)) .* harmochirp;
chirp_2 = 1.0 * (heaviside(t-2.5) - heaviside(t-3.5)) .* harmochirp;
